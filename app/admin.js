...
import { computeScores, maxForQuestion, sanitizeTest } from "./scoring.js";
...
async function viewResults() {
  ...
  const view = el(`
    <div class="glass pad appear">
      ...
      <div style="overflow-x:auto">
        <table>
          <thead><tr><th>Candidato</th><th>Enviado</th><th style="width:180px">Puntaje total</th><th class="hide-sm">Competencias</th><th></th></tr></thead>
          <tbody id="rows"></tbody>
        </table>
      </div>
    </div>`);
    ...
    const tr = el(`<tr class="row-click">
        ...
        <td>
          <div class="row between" style="margin-bottom:5px"><b style="color:${scoreColor(score.totalPct)}">${score.totalPct}%</b>
            <span class="tiny muted">${score.totalPoints}/${score.totalMax}${score.hasPendingManual ? " ·⏳" : ""}</span></div>
...
function openResult(s, TEST) {
  ...
  box.innerHTML = `
    ...
    <div class="glass" style="padding:20px;background:linear-gradient(135deg,rgba(255,128,0,.10),rgba(47,16,219,.08));margin-bottom:20px">
      ...
      ${score.hasPendingManual ? `<p class="tiny" style="color:#c05c00;margin-top:10px">⏳ Hay respuestas abiertas sin calificar. Asígnales puntaje abajo y guarda.</p>` : ""}
    </div>
